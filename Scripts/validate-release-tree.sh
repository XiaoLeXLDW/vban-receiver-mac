...
if [[ -d Tools ]]; then required_paths+=(Tools); fi
...
while IFS= read -r -d '' candidate; do
    ...
    find "${required_paths[@]}" \( -type f -o -type l \) -print0
)
...
if ! git diff --cached --quiet -- "${required_paths[@]}"; then
    fail "release inputs are staged but not committed; create the intended release commit first."
fi
